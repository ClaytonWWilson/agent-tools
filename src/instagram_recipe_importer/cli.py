...
def validate_url(ctx, param, value):
    ...
    if 'instagram.com' not in url_lower and 'youtube.com' not in url_lower and 'youtu.be' not in url_lower:
...
def main(url, output_dir, caption_only, no_thumbnail, verbose):
    ...
    if not caption_only:
        click.echo(f"Video downloaded: {'✅' if results['video_download'] else '❌'}", err=True)
        click.echo(f"Audio transcribed: {'✅' if results.get('transcription') else '❌'}", err=True)
        click.echo(f"OCR completed: {'✅' if results.get('ocr') else '❌'}", err=True)
    click.echo(f"Thumbnail downloaded: {'✅' if results.get('thumbnail') else '❌'}", err=True)
    click.echo("="*50, err=True)
